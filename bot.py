...
class Tomartod:
    ...
    def login(self, data):
        ...
        if res.status_code != 200:
            self.log(f"{merah}ủy quyền mã thông báo tìm nạp không thành công, hãy kiểm tra http.log !")
            return None
    ...
    def start_farming(self):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{merah}bắt đầu farming không thành công, hãy kiểm tra dòng cuối cùng của http.log !")
            return False
        ...
        self.log(f"{hijau}khởi đầu thành công farming !")
    ...
    def end_farming(self):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{merah}bắt đầu canh farming thành công, hãy kiểm tra dòng cuối cùng của http.log !")
            return False

        poin = res.json()["data"]["claim_this_time"]
        self.log(f"{hijau}Thành công claim farming !")
        self.log(f"{hijau}Phần thưởng : {putih}{poin}")
    ...
    def daily_claim(self):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{merah}yêu cầu không thành công, dấu hiệu hàng ngày, kiểm tra dòng cuối cùng của http.log !")
            return False

        data = res.json().get("data")
        if isinstance(data, str):
            self.log(f"{kuning}có thể đã đăng nhập rồi")
            return

        poin = data.get("today_points")
        self.log(
            f"{hijau}Thành công claim {biru}dấu hiệu hàng ngày {hijau}Phần thưởng : {putih}{poin} !"
        )
        return
    ...
    def play_game_func(self, amount_pass):
        ...
        for i in range(amount_pass):
            ...
            if res.status_code != 200:
                self.log(f"{merah}khởi đầu thất bại game !")
                return

            self.log(f"{hijau}Thành công {biru}Bắt đầu{hijau} game !")
            self.countdown(30)
            ...
            if res.status_code != 200:
                self.log(f"{merah}yêu cầu điểm game  không thành công !")
                continue

            self.log(f"{hijau}Thành công {biru}claim{hijau} game point : {putih}{point}")
    ...
    def get_balance(self):
        ...
        while True:
            ...
            if res.status_code != 200:
                self.log(f"{merah}tìm nạp không thành công balance !")
                continue
            data = res.json().get("data")
            if data is None:
                self.log(f"{merah}lấy dữ liệu thất bại !")
                return None
            ...
            if "farming" not in data.keys():
                self.log(f"{kuning}farming chưa bắt đầu !")
            ...
            self.log(f"{kuning}không có thời gian để claim !")
            self.log(f"{kuning}kết thúc farming tại : {putih}{format_end_farming}")
            if self.play_game:
                self.log(f"{hijau}trò chơi tự động chơi được kích hoạt !")
    ...
    def load_data(self, file):
        ...
        if len(datas) <= 0:
            print(
                f"{merah}0 tài khoản được phát hiện từ {file}, điền dữ liệu của bạn vào {file} Đầu tiên !{reset}"
            )
            sys.exit()
    ...
    def main(self):
        banner = f"""
    {hijau}Tự động Claim {biru}Tomarket_ai
    
    {hijau}By: {putih}t.me/Debugs
    {hijau}GIthub: {putih}@Debugs
    
    {hijau}Message: {putih}đừng quên 'git pull' có thể tập lệnh đã được cập nhật 
